...
////changes made by rajveer 14-10-2014
/******************************************************
******** Rectangle Generator ********
******************************************************/
...
/******************************************************
******** Semi Circle Generator ********
******************************************************/

$.fn.makeSemiCircle = function(config) {
    var SEM_isSemiCircleGenMaskKeyDown = false;
    var SEM_firstPoint = null;
    var SEM_secondPoint = null;
    var SEM_isFirstPointDrawn = false;
    var SEM_isSecondPointDrawn = false;

    $(document).keydown(function(evt) {
        switch (evt.keyCode) {
            case config.SemiCircleMaskKeyCode:
                SEM_isSemiCircleGenMaskKeyDown = true;
                break;
            default:
                break;
        }
    ...
    $(document).keyup(function(evt) {
        SEM_isSemiCircleGenMaskKeyDown = false;
    });

    $(this).dblclick(function(evt) {
        SEM_isFirstPointDrawn = false;
        SEM_isSecondPointDrawn = false;
        $(this).children("div.SEMcanvas").remove();
    });

    var SEMcanvasDiv;
    var SEMcanvasDiv_parentImg;
    var fstPtObj, sndPtObj;
    ...
    $(this).click(function(evt) {
        ...
        if (!SEM_isSemiCircleGenMaskKeyDown) {
            return;
        }
        ...
        imgTop = imgObj.offset().top;
        imgLeft = imgObj.offset().left;
        imgWidth = imgObj.width() * 2;
        imgHeight = imgObj.height();
        SEMcanvasDiv_parentImg = new ImageObject(imgWidth, imgHeight, imgLeft, imgHeight);

        SEMcanvasDiv = $(this).children("div.SEMcanvas");
        if (!SEMcanvasDiv.attr("class")) {
            SEMcanvasDiv = $("<div class=\"SEMcanvas\"></div>");
            SEMcanvasDiv.css({
                'left': imgLeft + 'px',
                'top': imgTop + 'px'
            });
            $(this).append(SEMcanvasDiv);
        }

        currentX = evt.pageX - imgLeft;
        currentY = evt.pageY - imgTop;

        if (currentX > SEMcanvasDiv_parentImg.ImgWidth || currentY > SEMcanvasDiv_parentImg.ImgHeight) {
            return;
        }

        if (!SEM_isFirstPointDrawn && !SEM_isSecondPointDrawn) {
            SEM_firstPoint = new Point(currentX, currentY);
            fstPtObj = drawCanvasPoint(SEMcanvasDiv, SEM_firstPoint, 'SEM_pt1', pointOffset);
            SEM_isFirstPointDrawn = true;
        }
        else if (SEM_isFirstPointDrawn && !SEM_isSecondPointDrawn) {
            if (currentX < SEM_firstPoint.X) {
                fstPtObj.remove();
                fstPtObj = null;
                SEM_isFirstPointDrawn = false;
                return;
            }
            SEM_secondPoint = new Point(currentX, currentY);
            sndPtObj = drawCanvasPoint(SEMcanvasDiv, SEM_secondPoint, 'SEM_pt2', pointOffset);
            SEM_isSecondPointDrawn = true;
        }

        if (SEM_isFirstPointDrawn && SEM_isSecondPointDrawn) {

        	drawSemiCircle(SEMcanvasDiv, SEM_firstPoint, SEM_secondPoint, imgObj.width());
...
function drawSemiCircle(canvasDiv, fstPt, sndPt, imgWid) {
    var tempHtml;
    heightVal = fstPt.Y - sndPt.Y;
    widthval = sndPt.X - fstPt.X;
    var imgWidth = imgWid - 3;	

    var radius = getSemiCircleRadius(fstPt, sndPt);    
    if (heightVal > 0) {
    	
		tempHtml += getSemiCircleHtmlForRadius(fstPt, sndPt, - radius, 0, 180, canvasDiv, "#0", imgWidth);
    }
    else {
        tempHtml += getSemiCircleHtmlForRadius(fstPt, sndPt, radius, 0, 180, canvasDiv, "#0", imgWidth);
    }
...
function getSemiCircleRadius(fstPt, sndPt) {
    var xDiff = sndPt.X - fstPt.X;
    var yDiff = sndPt.Y - fstPt.Y;
...
function getSemiCircleHtmlForRadius(fstPt, sndPt, radius1, startAngle, endAngle, parentImg, lineColor, imgWidth) {
    var incrementVal = 0;
    var tempHtml1 = "";
	...
	// X
    var startX = fstPt.X;
    var endX = 2 * sndPt.X - fstPt.X;
    var xStep = (endX - startX) / 180;

	// Y
    var startY = fstPt.Y;
    var endY = sndPt.Y;
    var yStep = Math.abs(startY - endY) / 90;
	
    var xVal = startX;
    var yVal = startY;
    var sign = 1;
    var a = (sndPt.Y - fstPt.Y);
    var b = Math.PI / (2 * (sndPt.X - fstPt.X));
    var c = -fstPt.X;
    var d = fstPt.Y;
    //alert(imgWidth);
    ...
    for (i = startAngle; i < endAngle; i += 1) {
        sinVal = Math.sin(Math.PI * i / 180);
        cosVal = Math.cos(Math.PI * i / 180);
    	//xVal = fstPt.X + Math.abs(i * cosVal);
        
    	//yVal = fstPt.Y + (radius1 * sinVal);
        //if (i > 90) sign = -1;
    	//yVal = yVal - sign * yStep;
    	//yVal = xVal * Math.sin(i);
        yVal = a * Math.sin(b * (xVal + c)) + d;
        xVal = xVal + xStep;
        
		
    	if (xVal > 0 && xVal < imgWidth) {
		
            tempHtml1 += "<div class='SEM_pe' style='left: " + xVal + "px; top: " + yVal + "px;background-color:" + lineColor + "'></div>";
         }
    }
